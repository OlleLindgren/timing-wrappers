...
class TimeKeeper:
    ...
    @staticmethod
    def get_col(exec_t_seconds: float|int) -> str:
    ...
    def str_format(dt: datetime.timedelta, name: str):
        ...
        return colored(f'{s:4d}:{mi:6d} | {name}', TimeKeeper.get_col(s))
    ...
    def tracks_str(cls):
        ...
        for key, records in sorted(cls.tracks.items(), key=lambda tup: -sum(tup[1])):
            ...
            result += colored(f'\n{total_s:4d}:{total_mi:6d} = {N:8d} * ({avg_s:4d}:{avg_mi:6d}) | {key}', TimeKeeper.get_col(total_s))
        return result
    ...
    def track(cls, key: str = '<None>'):
        cls.tracks[key] = cls.tracks.get(key, [])
        def pseudo_track(f: Callable):
            def wrapper(*args, **kwargs):
                t0 = time.time()
                result = f(*args, **kwargs)
                dt = time.time() - t0
                cls.tracks[key].append(dt)
                assert len(cls.tracks[key]) > 0
                return result
            return wrapper
        return pseudo_track
